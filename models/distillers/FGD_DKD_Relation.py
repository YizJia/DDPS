...
class FGD_DKD_Relation(Distiller):
    def __init__(self, cfg, student, teacher):
        super(FGD_DKD_Relation, self).__init__(student, teacher)
    ...
    def set_state_dict(self, ckpt_model):
        ...
        # load model for cross domain experiment
        save_model = ckpt_model["student"]
        state_dict = {k:v for k,v in save_model.items() if 'reid_loss' not in k}
        student_dict = self.student.state_dict()
        student_dict.update(state_dict)
