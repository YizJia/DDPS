...
from models.backbone_utils import build_resnet_fpn, build_mobilenet_fpn


class SeqNet_D(nn.Module):
    def __init__(self, cfg, model_cfg):
        super(SeqNet_D, self).__init__()

        # under backbone equal resnet + fpn
        backbone_name = model_cfg.BACKBONE_NAME
        if "resnet" in backbone_name:
            backbone = build_resnet_fpn(model_cfg)
        elif "mobilenet" in backbone_name:
            backbone = build_mobilenet_fpn(model_cfg)

        anchor_sizes = cfg.MODEL.ANCHOR_SIZES
        aspect_ratios = cfg.MODEL.ANCHOR_ASPECT_RATIOS * len(anchor_sizes)
        ...
        reid_head = TwoMLPHead(
            out_channels * resolution ** 2,
            representation_size
        )
        faster_rcnn_predictor = FastRCNNPredictor(representation_size, 2)
        box_predictor = BBoxRegressor(representation_size, num_classes=2, bn_neck=cfg.MODEL.ROI_HEAD.BN_NECK)
        roi_heads = SeqRoIHeads(
            ...
            # NAE
            nae_inchannels=[out_channels * resolution ** 2, 1024],
            # SeqNet
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False, **kwargs):
    ...
    def extract_infor_forKD(self, images, targets=None):
        """partly inference stage on teacher model"""
        features = self.backbone(images.tensors)
        proposals, _ = self.rpn(images, features, targets)
        detections, _ = self.roi_heads(
            features, proposals, images.image_sizes, targets)

        embedding_logits, box_labels, box_embeddings = self.roi_heads.extract_infor_forKD(features, detections, images.image_sizes, targets)
        return proposals, detections, embedding_logits, box_labels, box_embeddings
...
class SeqRoIHeads(RoIHeads):
    def __init__(
        ...
        self.embedding_head = NormAwareEmbedding(in_channels=nae_inchannels)
        ...
        # for KD
        self.select_proposals = []
        self.select_pro_labels = []
        self.proposals_logits = []
        self.select_boxes = []
        self.select_boxes_labels = []
        self.select_boxes_pid_labels = []
        self.boxes_logits = []

    def forward(self, features, proposals, image_shapes, targets=None, query_img_as_gallery=False, **kwargs):
        """
        Arguments:
        ...
        if self.training:
            ...
            # for KD
            self.select_proposals = proposals
        ...
        proposal_cls_scores, proposal_regs = self.faster_rcnn_predictor(
            proposal_features["feat_afFC"]
        )
        self.proposals_logits = proposal_cls_scores
        self.proposals_reg = proposal_regs
        ...
        if self.training:
            ...
            boxes, scores, labels = self.postprocess_proposals(
        ...
        if self.training:
            ...
            # for KD
            self.select_pro_labels = proposal_labels
            self.select_boxes_pid_labels = box_pid_labels
    ...
    def postprocess_boxes(
        ...
        if fcs is not None:
            ...
            pred_scores = torch.cat(fcs, dim=0)
        else:
        ...
        all_labels = []
        all_embeddings = []
        for boxes, scores, embeddings, labels, image_shape in zip(
                pred_boxes, pred_scores, pred_embeddings, box_labels, image_shapes
        ):
    ...
    def extract_infor_forKD(self, features, detections, image_shapes, targets=None, box_labels=None):
        boxes = [box_per_images["boxes"] for box_per_images in detections]
        if box_labels is None:
            ...
            _, box_labels = self.assign_targets_to_proposals(boxes, gt_boxes, gt_labels)

        box_features = self.box_roi_pool(features, boxes, image_shapes)
        box_features = self.reid_head(box_features)
        box_embeddings, _ = self.embedding_head(box_features)
        embedding_logits = self.reid_loss.get_soften_logits(box_embeddings, box_labels)
        return embedding_logits, box_labels, box_embeddings

    def get_select_proposals(self):
        return self.select_proposals, self.select_pro_labels

    def get_select_boxes(self):
        return self.select_boxes, self.select_boxes_pid_labels

    def get_logits(self):
        return self.proposals_logits, self.boxes_logits
    
    def get_proposal_regs(self):
        return self.proposals_reg

    def get_embeddings(self):
        return self.box_embeddings
...
class TwoMLPHead(nn.Module):
    ...
    def forward(self, x):
        ...
        fc = F.relu(self.fc6(x))
        fc = F.relu(self.fc7(fc))

        return OrderedDict([["feat_beFC", x], ["feat_afFC", fc]])
...
class NormAwareEmbedding(nn.Module):
    ...
    def __init__(self, featmap_names=["feat_beFC", "feat_afFC"], in_channels=[256, 1024], dim=256):
